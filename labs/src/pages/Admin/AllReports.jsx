...
const AllReports = () => {
  const { getLabAssignments } = useContext(LabsContext)
  ...
  const allReportsAssignments = useMemo(() => {
    const list = assignments.filter(a => a.status === 'Closed')
  ...
  return (
    <div className='m-5 w-full'>
      <div className='flex items-center justify-between gap-3 mb-4'>
        <p className='text-xl font-semibold text-gray-700'>All Reports</p>
      </div>
      ...
      <div className='bg-white border rounded-xl'>
        <div className='p-4'>
          ...
          />
          {allReportsAssignments.length === 0 ? (
            <p className='text-sm text-gray-400'>No collected reports.</p>
          ) : (
            <div className='space-y-3'>
              {allReportsAssignments.map((a) => (
