...
const Reports = () => {
  ...
  const filteredAssignments = useMemo(() => {
    const list = assignments.filter(a =>
      ['Pending Sample', 'Sample Collected', 'Reported', 'Closed'].includes(a.status)
    )
    if (!search.trim()) return sortByPriority(list)
    const q = search.trim().toLowerCase()
    return sortByPriority(list.filter(a =>
  ...
  const statusLabel = (status) => {
    if (status === 'Pending Sample') return 'Pending'
  ...
  const closeReport = async (assignmentId) => {
    const data = await updateLabAssignment({ assignmentId, status: 'Closed' })
    if (data.success) {
      toast.success('Report collected')
      refreshAssignments()
    } else {
...
<html>
  <head>
    <meta charset="utf-8"/>
    <title>Lab Report - ${assignment.patientName || assignment.patientId}</title>
    <style>
  ...
  return (
    <div className='m-5 w-full'>
      <div className='flex items-center justify-between gap-3 mb-4'>
        <p className='text-xl font-semibold text-gray-700'>Reports</p>
      </div>
          ...
          {filteredAssignments.length === 0 ? (
            ...
            <div className='space-y-3'>
              {filteredAssignments.map((a) => (
                <div key={a._id} className='border rounded-xl p-4 text-sm text-gray-600 flex flex-col md:flex-row md:items-center md:justify-between gap-3'>
                  <div>
                  ...
                  <div className='flex items-center gap-2'>
                    <span className='px-2 py-1 rounded-full text-xs bg-gray-100 text-gray-700'>{statusLabel(a.status)}</span>
                    {a.status === 'Reported' && (
                      <>
                        <button
                          onClick={async () => {
                            downloadReport(a)
                            await closeReport(a._id)
                          }}
