...
const updateProfile = async (req, res) => {
    ...
    try {
        ...
        const { userId, name, phone, address, dob, gender, age, height, weight, assignedDoctorId, assignedDoctorName } = req.body
        ...
        let computedAge = age
        if ((!computedAge || computedAge === 'NaN') && dob) {
            ...
            computedAge = Number.isFinite(years) ? years : null
        }
        ...
        if (computedAge !== undefined && computedAge !== null && computedAge !== '') updateData.age = Number(computedAge)
        if (height !== undefined && height !== null && height !== '') updateData.height = Number(height)
        if (weight !== undefined && weight !== null && weight !== '') updateData.weight = Number(weight)
...
// API to get user lab reports
const getLabReports = async (req, res) => {
    try {
        const { userId } = req.body
        const reports = await labAssignmentModel.find({ patientId: userId, status: { $in: ['Reported', 'Closed'] } }).sort({ reportGeneratedAt: -1 })
...
// API to get user pharmacy invoices
const getPharmacyInvoices = async (req, res) => {
    try {
        const { userId } = req.body
        const invoices = await pharmacyInvoiceModel.find({ userId }).sort({ createdAt: -1 })
