...
const FeesCatalog = () => {
  const { getFeesCatalog, updateFeesCatalog, feesCatalog } = useContext(AdminContext)
  ...
  useEffect(() => {
    const fetchCatalog = async () => {
      ...
      } else if (feesCatalog?.length) {
        setCategories(feesCatalog)
  ...
  const addCategory = () => {
    ...
    setCategories(prev => ([...prev, { key, name: '', items: [] }]))
  ...
  const onSave = async () => {
    ...
    setSaving(true)
    await updateFeesCatalog(normalizedCategories)
    setSaving(false)
  }
  ...
  return (
    <div className='m-5 w-full'>
      <div className='flex items-center justify-between mb-4'>
        <p className='text-xl font-semibold text-gray-700'>Fees Catalog</p>
        <div className='flex items-center gap-2'>
          <button onClick={addCategory} className='bg-primary text-white px-4 py-2 rounded-lg text-sm'>Add Category</button>
          <button onClick={onSave} className='bg-violet-600 text-white px-4 py-2 rounded-lg text-sm'>
            ...
            <div className='grid grid-cols-1 md:grid-cols-3 gap-3 items-end'>
              ...
              <div className='flex items-center gap-2'>
                {selectedCategoryIndex >= 0 && (
                  <button onClick={() => addItem(selectedCategoryIndex)} className='px-3 py-2 rounded-lg text-sm bg-violet-600 text-white'>Add Item</button>
                )}
                {selectedCategoryIndex >= 0 && (
                  <button onClick={() => removeCategory(selectedCategoryIndex)} className='px-3 py-2 rounded-lg text-sm bg-red-50 text-red-600'>Remove Category</button>
                )}
              </div>
            </div>
                ...
                <div className='mt-4 space-y-3'>
                  {filteredItems.length === 0 ? (
                    ...
                    filteredItems.map((item, itemIndex) => {
                      const actualIndex = (selectedCategory.items || []).findIndex(it => it === item)
                      return (
